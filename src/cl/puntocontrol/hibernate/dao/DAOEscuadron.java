...
public class DAOEscuadron 
{
	private static Log _log=LogFactory.getLog(DAOEscuadron.class);
	...
	public static void delete(String id_control) throws Exception {
		...
		try {
			...
			Escuadron obj = (Escuadron)session.get(
					Escuadron.class, id_control);
			if (obj != null) {
				session.delete(obj);
				session.flush();
			}
	 ...
	 * 
	 */
	public static void add(Escuadron objeto) throws Exception {
		Session session = null;
		try {
			session = HibernateSessionUtil.openSession();
			session.beginTransaction();
	 ...
	 * 
	 */
	public static Escuadron get(String id_control) throws Exception {
		Session session = null;

		try {
			session = HibernateSessionUtil.openSession();
			session.beginTransaction();
			Escuadron objeto = (Escuadron)session.get(
					Escuadron.class, id_control);
			session.getTransaction().commit();
			return objeto;
		}
	 ...
	 * 
	 */
	public static List<Escuadron> list(  String nombre_chofer
											,String patente
											,Date fechaDesde
											,Date fechaHasta
											,String rut_chofer
		...
		try {
			session = HibernateSessionUtil.openSession();
			session.beginTransaction();
			Criteria criteria=session.createCriteria(Escuadron.class);
	 ...
	 * 
	 */
	public static void update(Escuadron objeto) throws Exception {
		Session session = null;
