...
public class DAOPunto_Control 
{
	private static Log _log=LogFactory.getLog(DAOPunto_Control.class);
	...
	public static void delete(String id_control) throws Exception {
		...
		try {
			...
			Punto_Control obj = (Punto_Control)session.get(
					Punto_Control.class, id_control);
			if (obj != null) {
				session.delete(obj);
				session.flush();
			}
	 ...
	 * 
	 */
	public static void add(Punto_Control objeto) throws Exception {
		Session session = null;
		try {
			session = HibernateSessionUtil.openSession();
			session.beginTransaction();
	 ...
	 * 
	 */
	public static Punto_Control get(String id_control) throws Exception {
		Session session = null;

		try {
			session = HibernateSessionUtil.openSession();
			session.beginTransaction();
			Punto_Control objeto = (Punto_Control)session.get(
					Punto_Control.class, id_control);
			session.getTransaction().commit();
			return objeto;
		}
		catch (Exception e) {
			List<Punto_Control> l = list("", "", null, null, "", "", "", "", "", "", "", "", id_control);
			if(l!=null){
				if(l.get(0)!=null){
					return l.get(0);
				}
	 ...
	 * 
	 */
	public static List<Punto_Control> list(  String nombre_chofer
											,String patente
											,Date fechaDesde
											,Date fechaHasta
											,String rut_chofer
		...
		try {
			session = HibernateSessionUtil.openSession();
			session.beginTransaction();
			Criteria criteria=session.createCriteria(Punto_Control.class);
	 ...
	 * 
	 */
	public static void update(Punto_Control objeto) throws Exception {
		Session session = null;
